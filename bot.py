...
def getMessage():
    import sys
    print("📩 Telegram webhook пришёл!", file=sys.stdout)
    json_str = request.get_data().decode("UTF-8")
    update = types.Update.de_json(json_str)
    print("⚙️ update content:", update, file=sys.stdout)
    print("⚙️ update.message:", update.message, file=sys.stdout)
    print(json_str, file=sys.stdout)

    if update.message:
        bot.process_new_updates([update])
    else:
        print("⚠️ update.message is None", file=sys.stdout)
