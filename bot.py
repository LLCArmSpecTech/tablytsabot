...
def nova_poshta_back(message):
    ...
    # Проверка: был ли маршрут
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    if "machine" in user:  # если маршрут уже запущен
        keyboard.add("⛽ Заправка", "🏁 Конец маршрута", "📦 Новая почта")
    else:  # если маршрута не было
        keyboard.add("📦 Новая почта")
        keyboard.add("🚛 Начать маршрут")

    bot.send_message(chat_id, "Возврат в главное меню:", reply_markup=keyboard)
