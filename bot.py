...
def save_refuel_liters(message):
    ...
    # Шаг 1. Подготовка и конвертация
    liters_input = message.text.strip().replace(",", ".")  # Приводим к числу
    liters_float = float(liters_input)                     # Вычисления
    liters_display = liters_input.replace(".", ",")        # Для таблицы (с запятой)
    ...
    # Шаг 2. Запись в таблицу с запятой
    sheet.append_row([now, user["name"], liters_display, user["refuel_odometer"], str(consumption).replace(".", ",")])
